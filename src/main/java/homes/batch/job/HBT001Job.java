...
public class HBT001Job implements Job {
	
	public Logger Log = LogManager.getLogger(HBT001Job.class) ;

	private final BatchMapper mapper ; 

	public final String HBT001  = EnumBatchJob.MNG_LEDGR_MSTR.getCode() ; 
	public final String jobid   = EnumBatchJob.MNG_LEDGR_MSTR.getCode() ; 
	public final String jobnm   = EnumBatchJob.MNG_LEDGR_MSTR.getName() ;
	public final String bt_uuid = UUID.randomUUID().toString() ; 
	
	public BatchVo batchVo = new BatchVo(HBT001) ;
	...
	public int do_ready( CommonMap params ) {
		...
		LedgrMstrVo mstrVo = new LedgrMstrVo() ;
		mstrVo.setBatchde(batchde);
		mstrVo.setBatchAt(batchat);
		mstrVo.setMngrno(batchVo.getMngrno());
		int ex_co = mapper.getHbdLedgrMstrCount(mstrVo) ; 
	...
	public BatchVo do_work( CommonMap params ) {
		...
		mstrVo.setBatchAt(batchat);
		mstrVo.setMngrno(batchVo.getMngrno());
		int ex_co = mapper.insertLedgrMstr(mstrVo) ; 
		batchVo.setExco(ex_co) ;
		return this.batchVo ; 
	}
